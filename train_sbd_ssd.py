...
from gluoncv.data.transforms.presets.ssd import SSDNewTrainTransform
...
def parse_args():
    ...
    parser.add_argument('--save-prefix', type=str, default='train_ssd',
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers):
    ...
    batchify_fn = Tuple(*([Stack() for _ in range(5)]))
    with autograd.train_mode():
        _, _, anchors, _, _ = net(mx.nd.zeros((1, 3, height, width)))
    # batchify_fn = Tuple(Stack(), Stack(), Stack())  # stack image, cls_targets, box_targets
    train_loader = gluon.data.DataLoader(
        train_dataset.transform(SSDNewTrainTransform(width, height, anchors)),
        batch_size, True, batchify_fn=batchify_fn, last_batch='rollover', num_workers=num_workers)
    val_batchify_fn = Tuple(Stack(), Pad(pad_val=-1))
    val_loader = gluon.data.DataLoader(
        val_dataset.transform(SSDNewValTransform(width, height)),
...
def validate(net, val_data, ctx, eval_metric, polygon_metric):
...
def train(net, train_data, val_data, eval_metric, polygon_metric, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            coef_center_targets = gluon.utils.split_and_load(batch[3], ctx_list=ctx, batch_axis=0)
            coef_targets = gluon.utils.split_and_load(batch[4], ctx_list=ctx, batch_axis=0)
            ...
            with autograd.record():
                ...
                sum_loss, cls_loss, box_loss, coef_center_loss, coef_loss = mbox_loss(
                    cls_preds, box_preds, coef_center_preds, coef_preds, cls_targets, box_targets, coef_center_targets, coef_targets)
            ...
            coef_center_metric.update(0, [l * batch_size for l in coef_center_loss])
            ...
            if args.log_interval and not (i + 1) % args.log_interval:
                ...
                logger.info('[Epoch {}][Batch {}], Speed: {:.3f} samples/sec, LR:{}, {}={:.3f}, {}={:.3f}, {}={:.3f}, {}={:.3f}'.format(
                    epoch, i, batch_size/(time.time()-btic), trainer.learning_rate, name1, loss1, name2, loss2, name3, loss3, name4, loss4))
            btic = time.time()
        ...
        if (epoch+1) % args.val_interval == 0:
            ...
            map_bbox, map_polygon = validate(net, val_data, ctx, eval_metric, polygon_metric)
