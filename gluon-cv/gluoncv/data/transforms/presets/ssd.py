...
__all__ = ['transform_test', 'load_test', 'SSDDefaultTrainTransform', 'SSDDefaultValTransform', 'SSDNewTrainTransform', 'SSDNewValTransform']
...
class SSDNewTrainTransform(object):
    ...
    def __call__(self, src, label):
        ...
        gt_coef_center = mx.nd.array(bbox[np.newaxis, :, 4:6])
        gt_coef = mx.nd.array(bbox[np.newaxis, :, 6:6+2*self._deg+2])
        gt_ids = mx.nd.array(bbox[np.newaxis, :, 6+2*self._deg+2:7+2*self._deg+2])

        cls_targets, box_targets, coef_center_targets, coef_targets, _ = self._target_generator(
            self._anchors, None, gt_bboxes, gt_ids, gt_coef_center, gt_coef)
        return img, cls_targets[0], box_targets[0], coef_center_targets[0], coef_targets[0]


class SSDNewValTransform(object):
