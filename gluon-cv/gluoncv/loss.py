...
class SSDMultiBoxLoss(gluon.Block):
    ...
    def forward(self, cls_pred, box_pred, coef_center_pred, coef_pred, cls_target, box_target, coef_center_target, coef_target):
        ...
        if num_pos_all < 1:
            ...
            return nd.zeros((1,)), nd.zeros((1,)), nd.zeros((1,)), nd.zeros((1,)), nd.zeros((1,))
        ...
        coef_losses = []
        sum_losses = []
        for cp, bp, coefcp, coefp, ct, bt, coefct, coeft in zip(*[cls_pred, box_pred, coef_center_pred, coef_pred, cls_target,
            ...
            coef_center_loss = nd.where(coef_center_loss > self._rho, coef_center_loss - 0.5 * self._rho,
                                        (0.5 / self._rho) * nd.square(coef_center_loss))
            coef_center_loss = coef_center_loss * pos.expand_dims(axis=-1)
            coef_center_losses.append(nd.sum(coef_center_loss, axis=0, exclude=True) / num_pos_all)

            coefp = _reshape_like(nd, coefp, coeft)
            coef_loss = nd.abs(coefp, coeft)
            coef_loss = nd.where(coef_loss > self._rho, coef_loss - 0.5 * self._rho,
                                 (0.5 / self._rho) * nd.square(coef_loss))
            coef_loss = coef_loss * pos.expand_dims(axis=-1)
            coef_losses.append(nd.sum(coef_loss, axis=0, exclude=True) / num_pos_all)

            sum_losses.append(cls_losses[-1] + self._lambd * box_losses[-1] + coef_losses[-1] + coef_center_losses[-1])
