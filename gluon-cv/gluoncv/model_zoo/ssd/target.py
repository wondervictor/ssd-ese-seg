...
from ...nn.coder import MultiClassEncoder, NormalizedBoxCenterEncoder, NormalizedBoxCenterCOEFEncoder
...
class SSDTargetGenerator(Block):
    ...
    def __init__(self, iou_thresh=0.5, neg_thresh=0.5, negative_mining_ratio=3,
        ...
        self._box_encoder = NormalizedBoxCenterCOEFEncoder(stds=stds)
        self._center_to_corner = BBoxCenterToCorner(split=False)

    # pylint: disable=arguments-differ
    def forward(self, anchors, cls_preds, gt_boxes, gt_ids, gt_coef_center, gt_coef):
        ...
        box_targets, box_masks, coef_center, coef = self._box_encoder(samples, matches, anchors,
                                                                      gt_boxes, gt_coef_center, gt_coef)
        return cls_targets, box_targets, coef_center, coef, box_masks
