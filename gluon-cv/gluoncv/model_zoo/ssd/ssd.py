...
from ...nn.coder import MultiPerClassDecoder, NormalizedBoxCenterDecoder, NormalizedBoxCenterCOEFDecoder
...
class SSD(HybridBlock):
    ...
    def __init__(self, network, base_size, features, num_filters, sizes, ratios,
        ...
        with self.name_scope():
            ...
            for i, s, r, st in zip(range(num_layers), sizes, ratios, steps):
                ...
                self.box_predictors.add(ConvPredictor(num_anchors * (4 + 20)))
            self.bbox_decoder = NormalizedBoxCenterCOEFDecoder(stds)
    ...
    def hybrid_forward(self, F, x):
        ...
        reg_preds = F.concat(*box_preds, dim=1).reshape((0, -1, 24))
        box_preds = reg_preds
        anchors = F.concat(*anchors, dim=1).reshape((1, -1, 4))

        if autograd.is_training():
            box_preds = reg_preds.slice_axis(axis=-1, begin=0, end=4).reshape((0, -1, 4))
            coef_center_preds = reg_preds.slice_axis(axis=-1, begin=4, end=6).reshape((0, -1, 2))
            coef_preds = reg_preds.slice_axis(axis=-1, begin=6, end=24).reshape((0, -1, 18))
            return [cls_preds, box_preds, anchors, coef_center_preds, coef_preds]
        ...
        coef_centers = F.slice_axis(result, axis=2, begin=6, end=8)
        coef = F.slice_axis(result, axis=2, begin=8, end=26)
        return ids, scores, bboxes, coef_centers, coef
