...
class NormalizedBoxCenterCOEFEncoder(gluon.Block):
    ...
    def __init__(self, stds=(0.1, 0.1, 0.2, 0.2), means=(0., 0., 0., 0.)):
        super(NormalizedBoxCenterCOEFEncoder, self).__init__()
    ...
    def forward(self, samples, matches, anchors, refs, coef_center, coef):
        ...
        coef_center: (B, M, 2) coef centers
        coef: (B, M, 18) coef
        ...
        # [B, M, 2]
        ref_coef_centers = F.repeat(coef_center.reshape((0, 1, -1, 2)), axis=1, repeats=matches.shape[1])
        ref_coef_centers = F.split(ref_coef_centers, axis=-1, num_outputs=2, squeeze_axis=True)
        ref_coef_centers = F.concat(*[F.pick(ref_coef_centers[i], matches, axis=2).reshape((0, -1, 1)) \
            for i in range(2)], dim=2)

        coef[:, :, 0] = coef[:, :, 0] - 0.263289
        # [B, M, 18]
        ref_coef = F.repeat(coef.reshape((0, 1, -1, 18)), axis=1, repeats=matches.shape[1])
        ref_coef = F.split(ref_coef, axis=-1, num_outputs=18, squeeze_axis=True)
        ref_coef = F.concat(*[F.pick(ref_coef[i], matches, axis=2).reshape((0, -1, 1)) \
            for i in range(18)], dim=2)

        c = F.split(ref_coef_centers, axis=-1, num_outputs=2)
        ...
        # transform coef center based on bbox gt
        c0 = F.log((c[0] - (g[0] - g[2]/2)) / a[2])
        c1 = F.log((c[1] - (g[1] - g[3]/2)) / a[3])

        new_center = F.concat(c0, c1, dim=2)
        ...
        temp = F.tile(samples.reshape((0, -1, 1)), reps=(1, 1, 18)) > 0.5
        coef_ = F.where(temp, ref_coef, F.zeros_like(ref_coef))

        temp = F.tile(samples.reshape((0, -1, 1)), reps=(1, 1, 2)) > 0.5
        coef_center_ = F.where(temp, new_center, F.zeros_like(new_center))
        return targets, masks, coef_center_, coef_
...
class NormalizedBoxCenterCOEFDecoder(gluon.HybridBlock):
    ...
    def __init__(self, stds=(0.1, 0.1, 0.2, 0.2), means=(0., 0., 0., 0.),
        ...
        super(NormalizedBoxCenterCOEFDecoder, self).__init__()
        assert len(stds) == 4, "Box Encoder requires 4 std values."
        self._stds = stds
        self._means = means
        self._clip = clip
    ...
    def hybrid_forward(self, F, x, anchors):
        ...
        p = x.slice_axis(axis=-1, begin=0, end=4)
        coef_center = x.slice_axis(axis=-1, begin=4, end=6)
        coef = x.slice_axis(axis=-1, begin=6, end=24)

        p = F.split(p, axis=-1, num_outputs=4)
        c = F.split(coef_center, axis=-1, num_outputs=2)
        ...
        cx = F.broadcast_add(F.broadcast_mul(F.exp(c[0]), a[2]), ox-ow)
        cy = F.broadcast_add(F.broadcast_mul(F.exp(c[1]), a[3]), oy-oh)
        first_coef = coef.slice_axis(axis=-1, begin=0, end=1)
        other_coef = coef.slice_axis(axis=-1, begin=1, end=18)
        first_coef = F.broadcast_add(first_coef, F.ones_like(first_coef)*0.263289)
        coef = F.concat(first_coef, other_coef, dim=-1)

        return F.concat(ox - ow, oy - oh, ox + ow, oy + oh, cx, cy, coef, dim=-1)
